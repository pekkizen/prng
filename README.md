...
These conditions don't depend on the spacing and [0, 1) can
be divided subintervals of different, even or unven, spacings. 
...
For just to get a random float, the selection could be done the other
...
the smaller float, take the next bit and if it is '1', select the bigger one. 
...
So, also random_real.c implements the (modified) random bisecting algoritm.

...
equally likely float values in [0, 1). 
...
where f, fj and fj+1 are floats in the distribution and fj+1 is the next float to fj.
The single float point probability is 
- P(f = fj) = fj+1 - fj = Δj where
- Δj is one ULP in [2^-11, 1) and
- Δj is 2^-64 in [0, 2^-11) 
...
In P(f <= fj) = fj+1 the difference to standard uniform CDF is within
the normal floating point aritmetic rounding errors, eg.
- P(x <= 0.5) = 0.5 for standard uniform distribution and
- P(f <= 0.5) = 0.5 + 2^-53 = 0.5000000000000001 for Float64_64

The difference is one ULP and it is quite impossible to detect from the random stream of floats.
In the Float64_64 distribution every float has a probability equal to the 
space (Δj, mostly 1 ULP) it occupies in [0, 1) and we may call the distribution 
an unevenly spaced discrete uniform distribution, although this concept hardly exists.
...
This very likely implements the most accurate floating point approximation 
of the standard uniform distribution. 

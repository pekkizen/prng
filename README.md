...
an unevenly spaced discrete uniform distribution, where random variable f
...
version is slightly modified and can be found in xoro.go file.
...
if one try takes one nanosecond. After 405 more years we will have these zeros with 
probability 0.75. 2020 years give us a probability ~0.93.
...
random bisecting algoritm**. They are only 100 x faster, than the basic algoritm.

Also another view to the random bisecting float64 algorithm. 
The algorithm in fact isn't **"generating"** the floats, 
which are already "there" and are only waiting for to be picked up.

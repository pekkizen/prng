...
the rest 52 bits as float64 significand bits. If leading zeros >= 11, an evenly
...
has the same probability within all subintervals of the same size.
...
a 2.8 ns execution time, random number generation (xoroshiro128**) included. This 2.8 ns
version is slightly modified and can be found in the xoro.go file. Float64_64 is ~two
times slower than Float64, but from 1/2 to zero it is 2 - 2048 times more accurate..
...
```
Float64_1024 converts any random sequence of bits to a float64 in [0, 1).
...
All single floats (>= 2<sup>-1024</sup>) have a probability ULP of the float. 
...
http://prng.di.unimi.it/random_real.c. The main difference to Float64_1024 is, that
random_real.c rounds a 64-bit value to float64 in the final phase. Float64_1024 and
others use lossless float64 arithmetic with powers of two and dyadic rationals.
...
and repeat with the selected half. Stop when a desired accuracy is
reached.    

Float64_64Bisect implements the random bisecting for 64-bit random numbers.
It stops after 64 bits or when the interval is one ULP and cannot 
be splitted any more. The probability of a returned float is 1/2^bisectings. This is 
also the ULP of the float. To keep the same number value ordering in the bit sequence 
and the floats, the upper half is selected by a '1' bit and the lower half by a '0' bit.
From two adjacent floats, the smaller is always selected, which
ensures, that we never get a 1.

```Go
func Float64_64Bisect(u uint64) float64 {
	if u == 0 {
		return 0                       // without this, we never get a zero
	}
	left, mean, right := 0.0, 0.0, 1.0
	for i := 0; i < 64; i++ {
		mean = (left + right) / 2
...
```
In the interval [2<sup>-12</sup>, 1) Float64_64Bisect returns the same float as Float64_64.
When expanded to arbitrary (< ~1024) number of bits, it returns the same float as Float64_1024.
Expressed as another way: **Functions Float64_64 and Float64_1024 are implementations of the 
random bisecting algoritm**. They are just 100 x faster, than the basic algoritm 
function Float64_64Bisect.

Also another view to the random bisecting float64 algorithm. 
The algorithm in fact isn't **"generating"** the floats.
The about 1024 x 2<sup>52</sup> floats are already "there" and 
are only waiting for to be picked up.
The bit sequence key provides a unique path to find the float.
...
| (1) Float64_64/xoroshiro128** | 2.80    | 
| (1) Float64_1024/xoroshiro128** | 3.00    | 

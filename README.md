...
spaced float in [0, 2<sup>-11</sup>) is returned.
...
a 2.8 ns execution time, random number generation (xoroshiro128**) included.
...
All single floats (>= 2<sup>-1024</sup>) have a probability 1 ULP, a gap to the next float. 
...
We can pick an approximate real number from the standard uniform distribution by random bisecting:
- Split the interval [0, 1) into two equal length halves. Select either one by random
and repeat the process with the selected half. Stop when a desired accuracy is
reached.    
...
also the ULP of the float.

In the interval [2<sup>-12</sup>, 1) Float64_64Bisect returns the same float as Float64_64.
When expanded to arbitrary number of bits, it returns the same float as Float64_1024.
Expressed as another way: _Float64_64 and Float64_1024 are implementations of the 
random bisecting algoritm_.
...
func Float64_64Bisect(u uint64) float64 {
	...
	for i := 0; i < 64; i++ {
		...
		if u & (1 << 63) != 0 {
			left = mean                    // '1' bit -> take the right halv, big numbers
		} else {
			right = mean                   // '0' bit -> take the left halv, small numbers
		}
		u <<= 1
	}
	mean = (left + right) / 2

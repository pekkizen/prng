...
This distribution is called the _standard uniform distribution_. In a uniform 
distribution a random variable r has the same probability 
within all subintervals of the same size.     

Random numbers $k = 1, 2, ... n$, each with the probability $1/n$, can be mapped
to a _discrete uniform distribution_ of random variables $f_k$ in [0, 1) by 

$~~~~~~~~f_k = (k-1) / n,~~~f_k = 0, 1, ... (n-1)/n$


The random numbers $f_k$ have a CDF

$~~~~~~~~F(f) = P(f <= f_k) = f_k+1,~~for~ k <= n~ and~ f_n+1 = 1$

The probability of a single $f_k$ is       

$~~~~~~~~P(f = f_k) = P(f <= f_k) - P(f <= f_k-1) = f_k+1 - f_k = 1/n$

The probability of $f_k$ is the same as its distance to the next number in the distribution.
...
a probability equivalent to the space it occupies in [0, 1)** 
...
be divided subintervals with different, even or unven, spacings. The spacing
must be dense enough to meaningfully approximate the continuous distribution condition: 
subintervals of the same size have the same probability. This is for the case when subintervals 
are defined by arbitrary numbers in a computer program.

In measuring floating point number distances the concept of _ULP_ is generally used.
An ULP means a _unit of least precision_ or a _unit in the last place_ and one ULP is
the gap between two adjacent floats. Two adjacent floats differ only in the least significant bit.
The maximum ULP in [0, 1) is 1/2<sup>53</sup> in [1/2, 1).
The float64 numbers in [0, 1) are distributed so that 

- all intervals 
$[1/2^{k+1}, 1/2^k),~~~ 0 <= k <= 1021,$ have $2^{52}$ evenly spaced adjacent floats with an ULP $1/2^{53+k}.$ 
...
func Float64_64(urand uint64) float64 {
    ...
    f := float64((urand << zeros) >> 11) / (1 << 53)  // f goes first to [1/2, 1)
...
In P(f <= f<sub>k</sub>) = f<sub>k+1</sub> the the difference to the standard uniform CDF is
...
to the distribution, which is a nuisance to handle. 
...
They are only much more efficient code than Float64Bisect. 
Float64Bisect can return every float in [0, 1),
eg. the minimum subnormal positive value 2<sup>-1074</sup> and zeros after that.
...
of float64's in [0, 1). This is a rare case.

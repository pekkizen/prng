...
[Jean-Michel Muller (2005): On the definition of ulp(x)](http://ljk.imag.fr/membres/Carine.Lucas/TPScilab/JMMuller/ulp-toms.pdf).
...
is defined to be the distributon, where every float f<sub>k</sub>
is included and has the probability  ULP(f<sub>k</sub>).     
It is simple to draw random numbers from this distribution, even with fast functions. 
Here is a kind of point of view shift: drawing numbers from
a uniform distribution of floats vs. computing floating-point numbers 

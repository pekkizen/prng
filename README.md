...
As a property of a float it is (here) defined
...
implements an uniform distribution of all the normal floats in [0, 1).
...
Float64Bisect encodes this to a float64 by selecting the right half
with a '1' bit and the left half with a '0' bit. 
The random binary selection is essential, but
the interpreting the bit sequence to be anything is not.
...
Why this is exactly so, is harder to see. 

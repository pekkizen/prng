...
64-bit IEEE 754 floating point numbers, which are also referenced by _float64_.
...
This conditions doesn't depend on the spacing and [0, 1) can
...
of a float and 
the gap between a float and the next bigger adjacent float is one ULP. 
This directional ULP definition is needed for probabilities.
Two adjacent floats differ only in the least significant bit.
The maximum ULP in [0, 1) is 1/2<sup>53</sup> in [1/2, 1).
The float64 numbers in [0, 1) are "physically" exponentially distributed so that 
- all intervals 
...
is included and has a  probability equal to its ULP. 
...
seems to occur nowhere. The "producing random floats" is mostly described as to somehow 
compute a uniform "real number" in [0, 1) and then someway transform this to a floating point number in [0, 1).
...
which is the method generally used. Of the overall situation:
...
This is a discrete uniform distribution by a mathematical definition, not only
...
gives only every second float compared to the previous one. 
...
implements an uniform distribution of all float's in [0, 1).
...
All single floats (>= 2<sup>-1024</sup>) have a probability equal to its ULP. 
...
simulations these 64 zeros are likely to happen.
...
the smaller float, it can take the next bit and if it is '1', select the bigger one. 
...
Why this is so, is harder to see. 
The modified random bisecting give a 1 with probability 2<sup>-54</sup>.

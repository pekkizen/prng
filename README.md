...
Random numbers k = 1, 2, ... n, each with a probability 1/n, can be mapped
...
or otherwise, a discrete distribution in [0, 1) is uniform, 
if it has the CDF above. 
These conditions don't depend on the spacing. [0, 1) can
be divided subintervals with different, even or unven, spacings. 

In measuring floating point number distances the concept of _ULP_ is generally used.
An ULP means a _unit of least precision_ or a _unit in the last place_. It is a property
of a float64 number and 
the gap between two adjacent floats is one ULP. 
Two adjacent floats differ only in the least significant bit.
The maximum ULP in [0, 1) is 1/2<sup>53</sup> in [1/2, 1).
The float64 numbers in [0, 1) are distributed so that 
- all intervals 
[1/2<sup>k+1</sup>, 1/2<sup>k</sup>), &nbsp; 0 <= k <= 1021, 
have 2<sup>52</sup> evenly spaced adjacent floats with an ULP 1/2<sup>53+k</sup>. 

In the _**complete uniform distribution of float64's in [0, 1)**_ every float64 has a
probability equal to its ULP. This is a definition of a new concept.
...
float values in [0, 1). The maximun number of floats is 2<sup>53</sup> with
...
spaced float from [0, 2<sup>-11</sup>) is returned.
...
Besides, inside a computer each f<sub>k</sub> may contain rounding inaccuracy up to ULP/2.
...
This simple algorithm can produce every float64 (is complete) in 99.975% of the length of the unit interval 
...
All single floats (>= 2<sup>-1024</sup>) have a probability equal to the ULP of the float. 
...
if one try takes one nanosecond. However, in widely used applications these 64 zeros are
likely to happen.
...
be splitted any more. Every float in [0, 1) has a possibility to be selected. 
...
the complete uniform distribution of float64's in [0, 1). 
...
For a random float the selection could be done the other
way too, but this way we get the encoding as a byproduct.
...
They are only more efficient implementations than Float64Bisect. 
Float64Bisect can return every float in [0, 1),
eg. the minimum subnormal positive value 2<sup>-1074</sup> and a zero after that.

Float64Bisect can be modified to apply "rounding". Instead of always selecting 
the left value, take the next bit and if it is '1'
select the right value, otherwise left.  The modified Float64Bisect returns
the same float as random_real.c. The forcing the last bit to '1' by (u | 1)
in random_real.c is essential. So, also random_real.c implements 
the (modified) random bisecting algoritm.
The modified random bisecting gives a 1 with probability 2<sup>-54</sup>
and other floats in[1/2, 1) with probability 2<sup>-53</sup>. 
The probabilities of the floats in [0, 1) already sum to 1, so
the number 1 is a free bonus.
...
As an algoritm the binary search by bisecting is quite dumb. From the first
'1' bit it does 52 bisects and finally finds its own search key as the significand
bits of the float found. 
Nevertheless, with its few code lines it implements the complete uniform distribution 
of float64's in [0, 1). 
